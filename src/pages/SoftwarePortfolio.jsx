...
import { Github, Linkedin, Mail, Download, Moon, Sun, Send, Code2, Terminal, GraduationCap, BookOpen, CalendarDays, BarChart3 } from "lucide-react";
...
export default function SoftwarePortfolio(){
  ...
  return <div className="sw-shell">
    <header className="sw-nav glass">
      <a className="sw-brand" href="#home"><span><img src={profile} alt="" className="w-10 h-10 rounded-full"/></span><b>Sam Weslie Prabhakaran</b></a>
      <nav>{["Home","About","Projects","Stack","Education","Certifications","Contact"].map(x=><a key={x} href={`#${x.toLowerCase()}`} className="font-semibold">{x}</a>)}</nav>
      <div className="sw-nav-actions"><button className="theme-toggle" aria-label="Toggle color theme" onClick={()=>setTheme(theme==="dark"?"light":"dark")}><motion.span layout transition={{type:"spring",stiffness:400,damping:28}}>{theme==="dark"?<Moon size={15}/>:<Sun size={15}/>}</motion.span></button><a className="sw-btn compact" href="/Sam_Resume.pdf" download>Resume</a></div>
    </header>
    <main>
      <section id="home" className="sw-hero sw-section">
        <motion.div className="hero-copy" {...reveal}><p className="eyebrow">BUILD · SOLVE · IMPROVE</p><h1>Hi, I'm Sam Weslie<br/><span>Prabhakaran</span></h1><p className="hero-lead">Software Engineer building scalable software products. Skilled in both frontend and backend technologies, with writing clean and scalable code.</p><div className="hero-actions"><a className="sw-btn primary liquid" href="#projects">View my work <span>→</span></a><a className="sw-btn" href="/Sam_Resume.pdf" download><Download size={16}/> Download resume</a></div><div className="hero-meta"><div><strong>Java</strong><small>Backend</small></div><div><strong>React</strong><small>Frontend</small></div><div><strong>MySQL</strong><small>Database</small></div><div><strong>∞</strong><small>Always learning</small></div></div><div className="availability glass"><i/><div><b>Assistant System Engineer @ TCS</b><small>Chennai, Tamil Nadu.</small></div></div></motion.div>
        <motion.div className="dev-console" initial={{opacity:0,x:50}} animate={{opacity:1,x:0}} transition={{duration:.8,delay:.15}}><div className="editor glass liquid-card"><div className="window-bar"><span/><span/><span/><b>developer.js</b></div><pre><code><em>// Ideas into Reality</em>{`\n`}const ASE = {'{'}{`\n`} name: <mark>"Sam Weslie Prabhakaran"</mark>,{`\n`} role: <mark>"Software Engineer"</mark>,{`\n`} focus: ["Java", "React", "DBMS"],{`\n`} principle: <mark>"Clean code. Real impact."</mark>{`\n`}{'}'};</code></pre></div><div className="terminal glass liquid-card"><div className="window-bar"><Terminal size={14}/><b>sam@portfolio:~</b></div><p><span>$</span> npm run dev</p><p>✓ portfolio ready</p><p>✓ building · learning · shipping</p><p className="cursor">▌</p></div><div className="orb orb-a"/><div className="orb orb-b"/></motion.div>
      </section>

      <section id="about" className="sw-section sw-block"><motion.div {...reveal}><p className="eyebrow">01. ABOUT</p><h2>Engineering with curiosity.</h2><p className="section-copy">I’m Sam Weslie Prabhakaran, a Software Engineer focused on turning ideas into real world solutions. I enjoy understanding how systems work, solving real world problems, and continuously expanding my engineering knowledge.</p></motion.div><motion.div className="principles" {...reveal}>{[[Code2,"Problem solver"],[Terminal,"Clean-code mindset"],[GraduationCap,"Continuous learner"]].map(([Icon,t])=><div className="principle" key={t}><Icon/><b>{t}</b></div>)}</motion.div></section>
    ...
    <footer className="sw-footer"><div className="sw-brand"><span><img src={profile} alt="profile.png" className="w-10 h-10 rounded-full"/></span><b>Sam Weslie Prabhakaran</b></div><p>Building better tomorrow with technology</p><div><a href="https://github.com/ItsWeslie" target="_blank" rel="noreferrer"><Github/></a><a href="https://www.linkedin.com/in/samweslie14/" target="_blank" rel="noreferrer"><Linkedin/></a><a href="mailto:itssamwesliehere@gmail.com"><Mail/></a></div></footer>
  </div>;
}
