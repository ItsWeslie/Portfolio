...
const fallbackProjects = [
  ...
  },
  {
    ...
    category: "Full Stack",
    status: "Completed",
    image: airlineScreenshot,
  },
];

const filters = ["All", "Full Stack", "Java", "React", "MySQL"];

export default function ProjectRepositoryExplorer({
  ...
  const [terminalState, setTerminalState] = useState({
    ...
    output: `✓ ${projects.length} projects loaded\n✓ repository index ready`,
  });
  ...
  const visibleProjects = useMemo(
    () =>
      projects.filter((project) => {
        ...
        const matchesQuery =
          !q ||
          [project.title, project.slug, project.description, ...project.stack]
            .join(" ")
            .toLowerCase()
            .includes(q);
        const matchesFilter =
  ...
  const applyFilter = (nextFilter) => {
    ...
    const count = projects.filter(
      (project) =>
        nextFilter === "All" ||
        project.category === nextFilter ||
        project.stack.includes(nextFilter),
    ).length;
    setTerminalState({
      command: `filter --${nextFilter.toLowerCase().replaceAll(" ", "-")}`,
      output: `✓ ${count} repositories matched\n✓ repository index updated`,
    });
  };
  ...
  const handleSearch = (value) => {
    ...
    if (value.trim())
      setTerminalState({
        command: `search "${value.trim()}"`,
        output: "searching local project index...",
      });
    else
      setTerminalState({
        command: "filter --all",
        output: `✓ showing all ${projects.length} repositories`,
      });
  };
    ...
    <div className="project-explorer">
      <div className="project-intro">
        <div>
          ...
          <p className="section-copy">
            Software projects, experiments, and applications built to turn ideas
            into useful solutions.
          </p>
        </div>
        <div className="repo-summary glass">
          <div className="repo-summary-title">
            <FolderGit2 /> PROJECT REPOSITORY
          </div>
          <div className="repo-summary-grid">
            ...
            <div>
              <strong>{new Set(projects.flatMap((p) => p.stack)).size}</strong>
              <small>Technologies</small>
            </div>
            <div className="repo-summary-stack">
              <span>● Java</span>
              <span>● React</span>
              <span>● MySQL</span>
            </div>
          </div>
          <code>// BUILD → LEARN → IMPACT</code>
        </div>
      </div>

      <motion.div
        className="project-terminal glass"
      ...
      >
        <div className="project-terminal-bar">
          ...
          <span />
          <Terminal size={14} />
          <b>sam@portfolio:~/projects</b>
          <small>● ONLINE</small>
        </div>
        <div className="project-terminal-body">
          <div>
            <p>
              <i>$</i> {terminalState.command}
            </p>
            {terminalState.output.split("\n").map((line, i) => (
              <p className="terminal-output" key={`${line}-${i}`}>
                {line}
              </p>
            ))}
            <p>
              <i>$</i> <span className="terminal-cursor">▋</span>
            </p>
          </div>
          <blockquote>
            “Code builds more than software; it builds opportunities.”
          </blockquote>
        </div>
      </motion.div>
      ...
      <div className="repository-layout">
        <div className="repository-list" aria-label="Project repositories">
          {visibleProjects.length ? (
            visibleProjects.map((project, index) => (
              <motion.button
                type="button"
                className={`repository-row ${selected?.id === project.id ? "active" : ""}`}
              ...
              >
                ...
                <ChevronRight className="repo-chevron" size={18} />
              </motion.button>
            ))
          ) : (
            <div className="repository-empty">
              <Terminal />
              <b>No repositories matched.</b>
        ...
        <AnimatePresence mode="wait">
          {selected && (
            <motion.article
              key={selected.id}
              className="repository-detail glass"
            ...
            >
              <div className="repo-detail-top">
                ...
                {selected.featured && <b>Featured Project</b>}
              </div>
              ...
              <div className="repo-tech-row">
                {selected.stack.map((tag) => (
                  <span key={tag}>{tag}</span>
                ))}
              </div>
              <div className="project-screenshot-placeholder real-project-preview">
                ...
                {selected.image ? (
                  ...
                  <div className="missing-project-image">
                    <Terminal size={20} />
              ...
              <div className="repo-actions">
                {selected.liveUrl && (
                  <a
                    className="sw-btn primary liquid"
